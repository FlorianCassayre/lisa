...
object Printer {
     ...
     */
    def prettySCProof(proof: SCProof, showError: Option[(Seq[Int], String)] = None, forceDisplaySubproofs: Boolean = false): String = {
        ...
        lines.map {
            case (isMarked, indices, stepName, sequent) =>
                ...
                full.mkString(" ")
        }.mkString("\n") + (if (showError.nonEmpty) s"\nProof checker has reported an error at line ${prettyLineNumber(showError.get._1)}: ${showError.get._2}" else "")
    }
